...
@router.post(
    "/",
    response_model=CreateTextGenerationResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
async def create_generation_task(
    request: CreateTextGenerationRequest, db: AsyncSession = Depends(get_db)
):
    params = request.model_dump(exclude={"prompt", "model_name"}, exclude_none=True)
    ...
    generation_task.delay(generation.id, request.prompt, request.model_name, **params)
    return {"generation_id": generation.id}


@router.get(
    "/{gen_id}",
    response_model=GetTextGenerationResponse,

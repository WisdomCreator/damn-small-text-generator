...
def create_generation_task(request: CreateTextGenerationRequest, db: Session = Depends(get_db)):
...
@router.get("/{gen_id}", response_model=GetTextGenerationResponse)
def get_text_generation(gen_id: int, db: Session = Depends(get_db)):
